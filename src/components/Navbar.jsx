...
useEffect(() => {
  ...
  return (
<header
  className={`fixed top-0 z-50 w-full transition-all md:py-2 duration-500
    ${isScrolled === "transparent" ? "bg-transparent" : ""}
    ${isScrolled === "white" ? "bg-white" : ""}
    ${isScrolled === "black" ? "bg-black" : ""}
...
>
      ...
      <nav className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <div className="flex-shrink-0 2xl:ml-[-10rem]">
            <button>{isScrolled === "black" ? <LogoIconWhite /> : <LogoIcon />}</button>
          </div>

          {/* Desktop Menu */}
          <div className="hidden xl:flex items-center space-x-8 2xl:ml-[0rem] ml-0">

              <button
    key={navItems[0].label}
    className={`${isScrolled === "black" ? "text-white hover:bg-[#38393c]" : "text-[#15161a] hover:bg-[#d9edf7]"} rounded-full p-2 hover:cursor-pointer transition-all duration-300 ease-in-out font-xxthin`}
  >
    {navItems[0].label}
  </button>
            ...
            <div className="relative">
              <button
                ...
                className={`${isScrolled === "black" ? "text-white hover:bg-[#38393c]" : "text-[#15161a] hover:bg-[#d9edf7]"} flex items-center gap-1 rounded-full p-2 transition-all duration-300 ease-in-out hover:cursor-pointer font-xxthin`} 
              >
                Products {openDropdown === "products" ? <IoIosArrowUp /> : <IoIosArrowDown />}
              </button>
              {openDropdown === "products" && (
                <div className="absolute right-[-30rem] mt-1 w-[820px] bg-[#38393c] shadow-lg rounded-xl transition-all duration-300 ease-in-out grid grid-cols-2">
          ...
          {/* Right Buttons */}
          <div className="hidden xl:flex items-center space-x-4 2xl:ml-[10rem] ml-0">
            <button className="bg-[#292930] text-white py-[12px] px-[18px] rounded-full hover:bg-white hover:text-black transition-all duration-300 ease-in-out hover:cursor-pointer font-xxthin text-nowrap">
              Become a provider
            </button>
 <div className="relative">
  <button
    onClick={() => toggleDropdown("profile")}
    className="text-white hover:text-black hover:bg-white py-[12px] px-[18px] rounded-full bg-[#292930] transition-all duration-300 ease-in-out hover:cursor-pointer"
  >
    <ProfileIcon />
  </button>

  {openDropdown === "profile" && (
    <div className="absolute right-0 mt-4 w-[150px] bg-[#15161a] shadow-lg rounded-2xl transition-all duration-300 ease-in-out">
      {proItems.map((item, i) => (
        <button
          key={i}
          className="block w-full text-center px-1 py-2 hover:bg-[#292930] transition-all duration-300"
        >
          <p className="text-white text-[16px] leading-[22px] font-xxthin">{item}</p>
        </button>
      ))}
    </div>
  )}
          ...
          <div className="xl:hidden">
            <button
              ...
              className={`${isScrolled === "black" ? "text-white" : "text-[#15161a]"} text-gray-800 flex items-center hover:cursor-pointer gap-2`}
            >
              <FiMenu size={24} />
            </button>
          </div>
    ...
    >
      ...
      <div className="flex-shrink-0 w-full flex justify-end">
        <button
          ...
          className="text-white flex items-center hover:cursor-pointer py-4 px-12 gap-2 font-xxthin"
        >
      ...
      <div className="flex-1 overflow-y-auto scroll-hide px-4 pb-4 space-y-2">
        {navItems.map((item) => (
          <button
            key={item.label}
            className="text-[#d9edf7] text-left text-[18px] hover:cursor-pointer font-xxthin"
          >
            {item.label}
          </button>
        ))}
        ...
        {/* Products */}
        <button className="text-[#c8d7de] text-left text-[18px] hover:cursor-pointer font-xxthin">
          Products
        </button>
        {productItems.map((prod) => (
      ...
      <div className="flex-shrink-0 w-full flex items-center justify-between bg-[#38393c] md:py-[12px] px-2 md:px-8 py-2">
        <button className="bg-[#292930] text-[14px] md:text-[18px] text-white px-4 py-2 rounded-full hover:bg-white hover:text-black transition-all duration-300 font-xxthin">
          Become a provider
        </button>
